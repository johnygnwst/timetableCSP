...
def init_assignment_lcv_improved(csp):
    global var_domains
    global counter
    counter = 0
    assignment = {}
...
def backtracking_lcv_improved(assignment, csp, heuristic):
    global counter
    rnd_domains = csp[DOMAINS][:]

    while True:
        if is_complete(assignment):
            return assignment
        shuffle(rnd_domains)
        for value in rnd_domains:
...
def lcv_heuristic_improved(assignment):
    teach = {}
    for i in csp[VARIABLES]:
        teach[i._teacher] = 0
    res = []
